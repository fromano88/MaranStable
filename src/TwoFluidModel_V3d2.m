function TwoFluidModel_V3d2

evalin('base','clearvars'); close all; clc

f_main = figure('units','pixels','menubar','none','name',...
    'TWO FLUID MODEL','numbertitle','off','resize','off','Visible','Off');
...
set([h.gl.pm.liquid, h.gl.pm.gas],'call',{@gl_pm_fluid,h})
...
set([h.bc.et.axialLB.d1.temperature h.bc.et.axialLB.d2.temperature h.bc.et.radialLB.rc.temperature h.bc.et.axialSG.d1.temperature h.bc.et.axialSG.d2.temperature h.bc.et.radialSG.d1.temperature h.bc.et.radialSG.d2.temperature h.bc.et.radialSG.ro.temperature],'ButtonDownFcn',{@bc_pm_temperature,h})
...
if isunix && x > 1254 && x < 1288 && y > 730 && y < 756
    set(h.pl.info(1),'visible','on')
elseif ispc && x > 1223 && x < 1254 && y > 655 && y < 676
    set(h.pl.info(1),'visible','on')
else
...
if hObject ~= h.pb.bcs
    boundaryList = {h.b1.bc.rhs.T.z{1} h.b1.bc.rhs.T.r{1} h.b1.bc.rhs.T.r{2} h.b2.bc.rhs.T.z{1} h.b2.bc.rhs.T.z{3} h.b2.bc.rhs.T.z{4} h.b2.bc.rhs.T.r{1} h.b2.bc.rhs.T.r{2} h.b2.bc.rhs.w.r{2} h.b2.bc.rhs.w.r{1}};
    if max(cellfun(@isempty,boundaryList)) == 1
        waitfor(warndlg({'Boundary condition not defined!','', 'Set boundary condition before going on!'},'Warning 158'))
        return
    end
end

if ~isequal(hObject,h.pb.mesh) && ~isequal(hObject,h.pb.bcs)
...
switch hObject
    ...
    case {h.pb.mesh,h.pb.bcs}
        % this function only returns a context menu using a left click
            cm = uicontextmenu;
            if hObject == h.pb.mesh
                uimenu(cm,'Text','Liquid Phase','Callback',{@ce_pb_mesh,h});
                uimenu(cm,'Text','Gas Phase','Callback',{@ce_pb_mesh,h});
                ui_pos = [194 639];
            else
                uimenu(cm,'Text','Liquid Phase','Callback',{@ce_pb_bcs,h});
                uimenu(cm,'Text','Gas Phase','Callback',{@ce_pb_bcs,h});
                ui_pos = [374 639];
            end

        % context menu will be placed straight under the mesh button
            if isunix
                ui_pos = get(hObject,'Position');
            end
            set(cm,'Position',ui_pos(1:2));
            set(cm,'Visible','on');
...
set([h.mh.pl.radialLB.main h.mh.pl.axialLB.main h.mh.pl.radialSG.main h.mh.pl.axialSG.main],'visible','off')
set([h.mh.pb.drawMesh h.mh.pb.checkMesh],'visible','on');

if strcmp(hObject.Label,'Liquid Phase')
    % change headline
    set(h.mh.st.headline1,'string','Liquid Phase: Mesh Parameters in')

    % update visibilty & popup menu
    h.mh.Visible = 'LB';
    if strcmp(h.mh.visible.LB,'radial')
    ...
    % change headline
    set(h.mh.st.headline1,'string','Gas Phase: Mesh Parameters in')

    % update visibilty & popup menu
    h.mh.Visible = 'SG';
    if strcmp(h.mh.visible.SG,'radial')
        set(h.mh.pm.headline,'value',1)
        set(h.mh.pl.radialSG.main,'visible','on')
    else
        set(h.mh.pm.headline,'value',2)
        set(h.mh.pl.axialSG.main,'visible','on')
        set([h.mh.pb.drawMesh h.mh.pb.checkMesh],'visible','off');
    end
end

guidata(hObject, h);

function ce_pb_bcs(hObject, ~, h)
...
set([h.bc.pl.radialLB.main h.bc.pl.axialLB.main h.bc.pl.radialSG.main h.bc.pl.axialSG.main],'visible','off')
...
if strcmp(hObject.Label,'Liquid Phase')
    ...
    % update visibilty & popup menu
    h.bc.Visible = 'LB';
    if strcmp(h.bc.visible.LB,'radial')
    ...
    % update visibilty & popup menu
    h.bc.Visible = 'SG';
    if strcmp(h.bc.visible.SG,'radial')
        set(h.bc.pm.headline,'value',1)
        set(h.bc.pl.radialSG.main,'visible','on')
    else
        set(h.bc.pm.headline,'value',2)
        set(h.bc.pl.axialSG.main,'visible','on')
    end
end
...
switch hObject
    ...
    case h.gy.et.ld1
        h.lowerLimit=-inf; h.upperLimit=inf; name = 'l_d1';
        errormsg = 'Value must be positive.'; default = h.l_d1*1000; %#ok<NASGU>
    ...
    case h.gy.et.ld2
        h.lowerLimit=-inf; h.upperLimit=inf; name = 'l_d2';
        errormsg = 'Value must be positive.'; default = h.l_d2*1000; %#ok<NASGU>
        callbackName = 'gy_et_d';
    ...
    case h.gy.et.ri
        h.lowerLimit=h.r_c*1000; h.upperLimit=h.r_o*1000; name = 'r_i';
    ...
    case h.gy.et.ro
        h.lowerLimit=h.r_i*1000; h.upperLimit=inf; name = 'r_o';
        callbackName = 'gy_et_ro'; opts = h.opts; default = h.r_o*1000;  %#ok<NASGU>
        errormsg = ['Value must be greater than ' rx(p) '_i.']; %#ok<NASGU>
    ...
    case {h.mh.et.radialLB.spacing(1),h.mh.et.radialLB.spacing(2),...
        ...
        callbackName = 'mh_et_spacing';
        switch hObject
            case h.mh.et.radialLB.spacing(1)
                valueName = 'mesh.r.delta.start(1)';
            case h.mh.et.radialLB.spacing(2)
                valueName = 'mesh.r.delta.fit(1)';
            case h.mh.et.radialLB.spacing(3)
                valueName = 'mesh.r.delta.end(1)';
            case h.mh.et.radialSG.spacing(1)
                valueName = 'mesh.r.delta.start(2)';
            case h.mh.et.radialSG.spacing(2)
                valueName = 'mesh.r.delta.fit(2)';
            case h.mh.et.radialSG.spacing(3)
                valueName = 'mesh.r.delta.end(2)';
            case h.mh.et.axialSG.spacing(1)
                valueName = 'mesh.z.delta.start(1)';
            case h.mh.et.axialSG.spacing(2)
                valueName = 'mesh.z.delta.fit(1)';    
            case {h.mh.et.axialSG.spacing(3),h.mh.et.axialLB.spacing(1)}
                valueName = 'mesh.z.delta.start(2)';
            case {h.mh.et.axialSG.spacing(4),h.mh.et.axialLB.spacing(2)}
                valueName = 'mesh.z.delta.fit(2)';
            case {h.mh.et.axialSG.spacing(5),h.mh.et.axialLB.spacing(3)}
                valueName = 'mesh.z.delta.end(2)';
            case h.mh.et.axialSG.spacing(6)
                valueName = 'mesh.z.delta.fit(3)';
            case h.mh.et.axialSG.spacing(7)
                valueName = 'mesh.z.delta.end(3)';
        end
    ...
    case {h.mh.et.radialLB.stretchingC(1),h.mh.et.radialLB.stretchingC(3),...
        ...
        errormsg = 'Value must be larger than 1.'; %#ok<NASGU>
        switch hObject
            case h.mh.et.radialLB.stretchingC(1)
                valueName = 'mesh.r.f(1)';
            case h.mh.et.radialLB.stretchingC(3)
                valueName = 'mesh.r.f(2)';
            case h.mh.et.radialSG.stretchingC(1)
                valueName = 'mesh.r.f(3)';
            case h.mh.et.radialSG.stretchingC(3)
                valueName = 'mesh.r.f(4)';
            case h.mh.et.axialSG.stretchingC(1)
                valueName = 'mesh.z.f(1)';
            case {h.mh.et.axialSG.stretchingC(3),h.mh.et.axialLB.stretchingC(1)}
                valueName = 'mesh.z.f(2)';
            case {h.mh.et.axialSG.stretchingC(5),h.mh.et.axialLB.stretchingC(3)}
                valueName = 'mesh.z.f(3)';
            case h.mh.et.axialSG.stretchingC(7)
                valueName = 'mesh.z.f(4)';
        end
    ...
    case {h.bc.et.axialLB.d1.temperature h.bc.et.axialLB.d2.temperature ...
        ...
        switch hObject
            ...
            case h.bc.et.axialSG.d1.temperature
                valueName = 'b2.bc.rhs.T.r{1}';
            case h.bc.et.axialSG.d2.temperature
                valueName = 'b2.bc.rhs.T.r{2}';
            case h.bc.et.radialSG.d1.temperature
                valueName = 'b2.bc.rhs.T.z{1}';
            case h.bc.et.radialSG.d2.temperature
                valueName = 'b2.bc.rhs.T.z{3}';
            case h.bc.et.radialSG.ro.temperature
                valueName = 'b2.bc.rhs.T.z{4}';
        end
        h.vlNm = ['h.' valueName];
        default = eval(h.vlNm);
        if ~isempty(default)
    ...
    case {h.bc.et.axialSG.d1.velocity h.bc.et.axialSG.d2.velocity}
        callbackName = 'bc_et_velocity';
        switch hObject
            case h.bc.et.axialSG.d1.velocity
                h.lowerLimit=-inf; h.upperLimit=0;
                name = 'velocity_d1'; valueName = 'b2.bc.rhs.w.r{1}';
                errormsg = 'Value must be less than 0!'; %#ok<NASGU>
            case h.bc.et.axialSG.d2.velocity
                h.lowerLimit=0; h.upperLimit=inf;
                name = 'velocity_d2'; valueName = 'b2.bc.rhs.w.r{2}';
                errormsg = 'Value must be greater than 0!'; %#ok<NASGU>
        end
        h.vlNm = ['h.' valueName];
        
    case {h.sn.et.residuals(1),h.sn.et.residuals(2)}
        h.lowerLimit=1e-15; h.upperLimit=inf; name = 'residuals';
...
function h = ce_updateFunctions(h,velocityCorrection)
...
% update every temperature profile in case the profile contains this geometry parameter
    ...
    if strcmp(h.b2.bc.z(1,3),'c')
        h.b2.bc.rhs.T.z{1} = get(h.bc.et.radialSG.d1.temperature,'string');
        h.b2.bc.rhs.T.z{1} = convertTemperature(h,h.b2.bc.rhs.T.z{1},'z');
    end
    % --------------------------------------------------------------------%
    if strcmp(h.b2.bc.z(3,3),'c')
        h.b2.bc.rhs.T.z{3} = get(h.bc.et.radialSG.d2.temperature,'string');
        h.b2.bc.rhs.T.z{3} = convertTemperature(h,h.b2.bc.rhs.T.z{3},'z');
    end
    % --------------------------------------------------------------------%
    if strcmp(h.b2.bc.z(4,3),'c')
        h.b2.bc.rhs.T.z{4} = get(h.bc.et.radialSG.ro.temperature,'string');
        h.b2.bc.rhs.T.z{4} = convertTemperature(h,h.b2.bc.rhs.T.z{4},'z');
    end
    % --------------------------------------------------------------------%
    if strcmp(h.b2.bc.r(1,3),'c')
        h.b2.bc.rhs.T.r{1} = get(h.bc.et.axialSG.d1.temperature,'string');
        h.b2.bc.rhs.T.r{1} = convertTemperature(h,h.b2.bc.rhs.T.r{1},'r');
    end
    % --------------------------------------------------------------------%
    if strcmp(h.b2.bc.r(2,3),'c')
        h.b2.bc.rhs.T.r{2} = get(h.bc.et.axialSG.d2.temperature,'string');
        h.b2.bc.rhs.T.r{2} = convertTemperature(h,h.b2.bc.rhs.T.r{2},'r');
    end
    
if velocityCorrection == 1
    % update velocity profiles in case the profile contains this geometry parameter
    if strcmp(h.b2.bc.r(1,1),'i')
        h.b2.bc.rhs.w.r{1} = get(h.bc.et.axialSG.d1.velocity,'string');
        h.b2.bc.rhs.w.r{1} = convertVelocity(h,h.b2.bc.rhs.w.r{1});
    end
    % --------------------------------------------------------------------%
    if strcmp(h.b2.bc.r(2,1),'i')
        h.b2.bc.rhs.w.r{2} = get(h.bc.et.axialSG.d2.velocity,'string');
        h.b2.bc.rhs.w.r{2} = convertVelocity(h,h.b2.bc.rhs.w.r{2});
    end
end
...
updateValues_TFM; evalin('base','caseSetup_TFM_V3d2')
...
bc_list = {'h.b1.bc.rhs.T.r{1}', 'h.b1.bc.rhs.T.r{2}', 'h.b2.bc.rhs.T.z{1}',...
           'h.b2.bc.rhs.T.z{3}', 'h.b2.bc.rhs.T.r{1}', 'h.b2.bc.rhs.T.r{2}',...
           'h.b2.bc.rhs.T.z{4}', 'h.b1.bc.rhs.T.z{1}'};
...
switch str{val}
    case 'Others ...'
        if h.gl.pm.liquid == hObject
            ...
            selectedFluid = getFluid('gas',h);
            if ~isempty(selectedFluid)
                h.selectedGas = selectedFluid;
            else
                selectedFluid = h.selectedGas;
            end
        end
...
if h.flowopt.energy == 1
    ...
    if isfile([tempdir 'my_fluids.mat'])
        ...
        if max(ismember(myfluids,h.selectedLiquid)) || max(ismember(myfluids,h.selectedGas))
            if h.NS == 3
...
[name, prop, fluid] = createFluid('TFM',h); %#ok<ASGLU> 
if isempty(name)
    return
end
if strcmp(fluid,'liquid')
    ...
    h.gas_list = {name, h.gas_list{1:end}};
    h.selectedGas = name;
    str = get(h.gl.pm.gas,'String');
    str = [name; str];
    set(h.gl.pm.gas,'String',str,'Value',1)
end
...
if str2double(get(hObject,'string')) <= 0
    if h.show.(name).warning == 1
        waitfor(warndlg('Value must be positive.','Attention'))
        h.show.(name).warning = 0;
    end
    val = 0.1; set(hObject,'string',num2str(val));
    h.(name) = val/1000;
end

switch hObject
    case h.gy.et.ld1
        updated = 'l_d1'; %#ok<NASGU>

    case h.gy.et.llb
        updated = 'l_lb'; %#ok<NASGU>

    case h.gy.et.ld2
        updated = 'l_d2'; %#ok<NASGU>
end    
...
if h.r_c == h.r_i
    ...
    set(hObject,'string',num2str(h.r_i*1000));
    set(h.gy.et.ro,'String',num2str(h.r_o*1000));
end
    
% changing value if r_i == r_o
if h.r_i == h.r_o
    if h.r_i > h.lowerLimit + 0.1
        h.r_i = h.r_i - 0.1/1000;
    elseif h.r_c > 0.1/1000
        h.r_i = h.r_o - 0.1/1000;
        h.r_c = h.r_i - 0.1/1000;
    else
        h.r_i = h.r_c + 0.1/1000;
        h.r_o = h.r_i + 0.1/1000;
    end
    set(h.gy.et.rc,'String',num2str(h.r_c*1000));
    set(hObject,'string',num2str(h.r_i*1000));
    set(h.gy.et.ro,'String',num2str(h.r_o*1000));
end
    
h = ce_updateFunctions(h,1);
...
% changing value if r_i == r_o
    if h.r_o == h.r_i
        h.r_o = h.r_o + 0.1/1000;
        set(hObject,'string',num2str(h.r_o*1000));
    end
...
switch val
    ...
    case 2
        if strcmp(h.mh.Visible,'LB')
            set(h.mh.pl.axialLB.main, 'visible', 'on')
            h.mh.visible.LB = 'axial';
        else
            set(h.mh.pl.axialSG.main, 'visible', 'on')
            set([h.mh.pb.drawMesh h.mh.pb.checkMesh],'visible','off');
            h.mh.visible.SG = 'axial';
        end
end

guidata(hObject,h)

function mh_et_spacing(hObject, ~, ~)
h = guidata(hObject);

switch hObject
    case {h.mh.et.axialLB.spacing(1),h.mh.et.axialSG.spacing(3)}
        h.mesh.z.delta.end(1) = h.mesh.z.delta.start(2);
        set([h.mh.et.axialLB.spacing(1) h.mh.et.axialSG.spacing(3)],...
            'string',num2str(h.mesh.z.delta.start(2)))
        
    case {h.mh.et.axialLB.spacing(2),h.mh.et.axialSG.spacing(4)}
        set([h.mh.et.axialLB.spacing(2) h.mh.et.axialSG.spacing(4)],...
            'string',num2str(h.mesh.z.delta.fit(2)))
        
    case {h.mh.et.axialLB.spacing(3),h.mh.et.axialSG.spacing(5)}
        h.mesh.z.delta.start(3) = h.mesh.z.delta.end(2);
        set([h.mh.et.axialLB.spacing(3) h.mh.et.axialSG.spacing(5)],...
            'string',num2str(h.mesh.z.delta.end(2)))
end
...
radial_gp = get(h.mh.rb.radialLB(1).gp,'value') + get(h.mh.rb.radialLB(3).gp,'value') + get(h.mh.rb.radialSG(1).gp,'value') + get(h.mh.rb.radialSG(3).gp,'value');
axial_gp = get(h.mh.rb.axialLB(1).gp,'value') + get(h.mh.rb.axialLB(3).gp,'value') + get(h.mh.rb.axialSG(1).gp,'value') + get(h.mh.rb.axialSG(3).gp,'value') + get(h.mh.rb.axialSG(5).gp,'value') + get(h.mh.rb.axialSG(7).gp,'value');
...
switch hObject
    ...
    case {h.mh.et.axialSG.stretchingC(3),h.mh.et.axialLB.stretchingC(1)}
        set([h.mh.et.axialLB.stretchingC(1) h.mh.et.axialSG.stretchingC(3)],'string',num2str(h.mesh.z.f(2)))
        if h.mesh.z.f(2) == 1
    ...
    case {h.mh.et.axialSG.stretchingC(5),h.mh.et.axialLB.stretchingC(3)}
        set([h.mh.et.axialLB.stretchingC(3) h.mh.et.axialSG.stretchingC(5)],'string',num2str(h.mesh.z.f(3)))
        if h.mesh.z.f(3) == 1
...
switch selection
    case 'Hyperbolic Tangent'
        ...
        if get(h.mh.rb.radialSG(1).lin,'Value') == 0
            set(h.mh.rb.radialSG(1).tanh,'Value',1);
        end
        if get(h.mh.rb.radialSG(3).lin,'Value') == 0
            set(h.mh.rb.radialSG(3).tanh,'Value',1);
        end
        h.mesh.r.sf={'tanh'};
    ...
    case 'Linear'
        if h.show.stretchingF_lin == 1
            helpdlg('Corresponding Stretching Factor f set to 1!','Notice')
        ...
        switch selection
            case {'Hyperbolic Tangent','Geometric Progression'}
                ...
                if h.mesh.r.f(3) == 1
                    ...
                    h.mesh.r.f(3) = default;
                    set(h.mh.et.radialSG.stretchingC(1),'String',num2str(default))
                    set(h.mh.et.radialSG.spacing(1),'enable','on')
                end

            case 'Linear'
                h.mesh.r.f(3) = 1;
                set(h.mh.et.radialSG.stretchingC(1),'String','1')
                set(h.mh.et.radialSG.spacing(1),'enable','off')
        end
        
    case h.mh.bg.radialSG(3)
        switch selection
            case {'Hyperbolic Tangent','Geometric Progression'}
                if h.mesh.r.f(4) == 1
                    if h.show.stretchingF == 1
                    ...
                    h.mesh.r.f(4) = default;
                    set(h.mh.et.radialSG.stretchingC(3),'String',num2str(default))
                    set(h.mh.et.radialSG.spacing(3),'enable','on')
                end

            case 'Linear'
                h.mesh.r.f(4) = 1;
                set(h.mh.et.radialSG.stretchingC(3),'String','1')
                set(h.mh.et.radialSG.spacing(3),'enable','off')
        end
...
switch selection
    case 'Hyperbolic Tangent'
        ...
        if get(h.mh.rb.axialSG(1).lin,'Value') == 0
            set(h.mh.rb.axialSG(1).tanh,'Value',1);
        end
        if get(h.mh.rb.axialSG(3).lin,'Value') == 0
            set(h.mh.rb.axialSG(3).tanh,'Value',1);
        end
        if get(h.mh.rb.axialSG(5).lin,'Value') == 0
            set(h.mh.rb.axialSG(5).tanh,'Value',1);
        end
        if get(h.mh.rb.axialSG(7).lin,'Value') == 0
            set(h.mh.rb.axialSG(7).tanh,'Value',1);
        end
        h.mesh.z.sf={'tanh'};
    ...
    case 'Geometric Progression'
        ...
        if get(h.mh.rb.axialSG(3).lin,'Value') == 0
            set(h.mh.rb.axialSG(3).gp,'Value',1);
        end
        if get(h.mh.rb.axialSG(5).lin,'Value') == 0
            set(h.mh.rb.axialSG(5).gp,'Value',1);
        end
        if get(h.mh.rb.axialSG(7).lin,'Value') == 0
            set(h.mh.rb.axialSG(7).gp,'Value',1);
        end
        ...
        switch selection
            case {'Hyperbolic Tangent','Geometric Progression'}
                if h.mesh.z.f(1) == 1
                    ...
                    set(h.mh.et.axialSG.stretchingC(1),'String',num2str(default))
                    set(h.mh.et.axialSG.spacing(1),'enable','on')
                end

            case 'Linear'
                h.mesh.z.f(1) = 1;
                set(h.mh.et.axialSG.stretchingC(1),'String','1')
                set(h.mh.et.axialSG.spacing(1),'enable','off')
        end
        
    case {h.mh.bg.axialSG(3),h.mh.bg.axialLB(1)}
        switch selection
            case 'Hyperbolic Tangent'
                set([h.mh.rb.axialLB(1).tanh h.mh.rb.axialSG(3).tanh],'Value',1);
                if h.mesh.z.f(2) == 1
                    ...
                    set([h.mh.et.axialLB.stretchingC(1) h.mh.et.axialSG.stretchingC(3)],'String',num2str(default))
                    set([h.mh.et.axialLB.spacing(1) h.mh.et.axialSG.spacing(2) h.mh.et.axialSG.spacing(3)],'enable','on')
                end
            ...
            case 'Linear'
                h.mesh.z.f(2) = 1;
                set([h.mh.et.axialLB.stretchingC(1) h.mh.et.axialSG.stretchingC(3)],'String','1')
                set([h.mh.rb.axialLB(1).lin h.mh.rb.axialSG(3).lin],'Value',1);
                set([h.mh.et.axialLB.spacing(1) h.mh.et.axialSG.spacing(2) h.mh.et.axialSG.spacing(3)],'enable','off')
        end
        
    case {h.mh.bg.axialSG(5),h.mh.bg.axialLB(3)}
        switch selection
            case 'Hyperbolic Tangent'
                set([h.mh.rb.axialLB(3).tanh h.mh.rb.axialSG(5).tanh],'Value',1);
                if h.mesh.z.f(3) == 1
                    ...
                    h.mesh.z.f(3) = default;
                    set([h.mh.et.axialLB.stretchingC(3) h.mh.et.axialSG.stretchingC(5)],'String',num2str(default))
                    set([h.mh.et.axialLB.spacing(3) h.mh.et.axialSG.spacing(5) h.mh.et.axialSG.spacing(6)],'enable','on')
                end
            ...
            case 'Linear'
                h.mesh.z.f(3) = 1;
                set([h.mh.et.axialLB.stretchingC(3) h.mh.et.axialSG.stretchingC(5)],'String','1')
                set([h.mh.rb.axialLB(3).lin h.mh.rb.axialSG(5).lin],'Value',1);
                set([h.mh.et.axialLB.spacing(3) h.mh.et.axialSG.spacing(5) h.mh.et.axialSG.spacing(6)],'enable','off')
        end
        
    case h.mh.bg.axialSG(7)
        switch selection
            case {'Hyperbolic Tangent','Geometric Progression'}
                if h.mesh.z.f(4) == 1
                    if h.show.stretchingF == 1
                    ...
                    h.mesh.z.f(4) = default;
                    set(h.mh.et.axialSG.stretchingC(7),'String',num2str(default))
                    set(h.mh.et.axialSG.spacing(7),'enable','on')
                end

            case 'Linear'
                h.mesh.z.f(4) = 1;
                set(h.mh.et.axialSG.stretchingC(7),'String','1')
                set(h.mh.et.axialSG.spacing(7),'enable','off')
        end
end
...
% run the setup needed for all computations
    try
        evalin('base','caseSetup_TFM_V3d2')
    catch
        close(wb)
...
switch val
    case 1
        if strcmp(h.bc.Visible,'LB')
            set(h.bc.pl.radialLB.main,'visible','on')
            h.bc.visible.LB = 'radial';
        else
            set(h.bc.pl.radialSG.main,'visible','on')
            h.bc.visible.SG = 'radial';
        end
    case 2
        if strcmp(h.bc.Visible,'LB')
            set(h.bc.pl.axialLB.main,'visible','on')
...
switch selection
    ...
    case 'Conductive'
        switch hObject
            ...
            case h.bc.bg.axialSG.d1(3)
                h.b2.bc.r(1,3) = 'c';
                if isempty(get(h.bc.et.axialSG.d1.temperature,'String'))
                    h.b2.bc.rhs.T.r{1} = [];
                else
                    h.b2.bc.rhs.T.r{1} = get(h.bc.et.axialSG.d1.temperature,'string');
                    h.b2.bc.rhs.T.r{1} = convertTemperature(h,h.b2.bc.rhs.T.r{1},'r');
                ...
                set([h.bc.et.axialSG.d1.temperature h.bc.pm.axialSG.d1.temperature h.bc.st.axialSG.d1.temperature],'visible','on')
                axes(h.bc.as.axialSG.d1(2));
                text(0.94*h.tp_r,0.47,'$^\circ$C',h.latex_r{:})
                if get(h.bc.pm.axialSG.d1.temperature,'value') == 2
                    text(0.4*h.tp_l,0.47,['$T_{1}(' rx(p) ') =$'],h.latex_l{:})
                else
                    text(0.4*h.tp_l,0.47,'$T_{1} =$',h.latex_l{:})
                end
            ...
            case h.bc.bg.axialSG.d2(3)
                h.b2.bc.r(2,3) = 'c';
                if isempty(get(h.bc.et.axialSG.d2.temperature,'String'))
                    h.b2.bc.rhs.T.r{2} = [];
                else
                    h.b2.bc.rhs.T.r{2} = get(h.bc.et.axialSG.d2.temperature,'string');
                    h.b2.bc.rhs.T.r{2} = convertTemperature(h,h.b2.bc.rhs.T.r{1},'r');
                end

                set([h.bc.et.axialSG.d2.temperature h.bc.pm.axialSG.d2.temperature h.bc.st.axialSG.d2.temperature],'visible','on')
                axes(h.bc.as.axialSG.d2(2));
                text(0.94*h.tp_r,0.47,'$^\circ$C',h.latex_r{:})
                if get(h.bc.pm.axialSG.d2.temperature,'value') == 2
                    text(0.4*h.tp_l,0.47,['$T_{2}(' rx(p) ') =$'],h.latex_l{:})
                else
                    text(0.4*h.tp_l,0.47,'$T_{2} =$',h.latex_l{:})
                end
            ...
            case h.bc.bg.radialSG.d1(3)
                h.b2.bc.z(1,3) = 'c';
                if isempty(get(h.bc.et.radialSG.d1.temperature,'String'))
                    h.b2.bc.rhs.T.z{1} = [];
                else
                    h.b2.bc.rhs.T.z{1} = get(h.bc.et.radialSG.d1.temperature,'string');
                    h.b2.bc.rhs.T.z{1} = convertTemperature(h,h.b2.bc.rhs.T.z{1},'z');
            ...
            case h.bc.bg.radialSG.d2(3)
                h.b2.bc.z(3,3) = 'c';
                if isempty(get(h.bc.et.radialSG.d2.temperature,'String'))
                    h.b2.bc.rhs.T.z{3} = [];
                else
                    h.b2.bc.rhs.T.z{3} = get(h.bc.et.radialSG.d2.temperature,'string');
                    h.b2.bc.rhs.T.z{3} = convertTemperature(h,h.b2.bc.rhs.T.z{1},'z');
                end

                set([h.bc.et.radialSG.d2.temperature h.bc.pm.radialSG.d2.temperature h.bc.st.radialSG.d2.temperature],'visible','on')
                axes(h.bc.as.radialSG.d2(2))
                text(0.94*h.tp_r,0.47,'$^\circ$C',h.latex_r{:})
            ...
            case h.bc.bg.radialSG.ro(3)
                h.b2.bc.z(4,3) = 'c';
                if isempty(get(h.bc.et.radialSG.ro.temperature,'String'))
                    h.b2.bc.rhs.T.z{4} = [];
                else
                    h.b2.bc.rhs.T.z{4} = get(h.bc.et.radialSG.ro.temperature,'string');
                    h.b2.bc.rhs.T.z{4} = convertTemperature(h,h.b2.bc.rhs.T.z{1},'z');
                end

                set([h.bc.et.radialSG.ro.temperature h.bc.pm.radialSG.ro.temperature h.bc.st.radialSG.ro.temperature],'visible','on')
                axes(h.bc.as.radialSG.ro(2))
                text(0.94*h.tp_r,0.47,'$^\circ$C',h.latex_r{:})
...
switch selection
    case 'No Slip'
        switch hObject
            ...
            case h.bc.bg.axialSG.d1(2)
                h.b2.bc.r(1,2) = 'n';

            case h.bc.bg.axialSG.d2(2)
                h.b2.bc.r(2,2) = 'n';

            case h.bc.bg.radialSG.d1(2)
                h.b2.bc.z(1,2) = 'n';

            case h.bc.bg.radialSG.d2(2)
                h.b2.bc.z(3,2) = 'n';

            case h.bc.bg.radialSG.ro(2)
                h.b2.bc.z(4,2) = 'n';
        end
...
if (strcmp(selection,'Inflow') && hObject == h.bc.bg.axialSG.d1(1)) || (strcmp(selection,'Outflow') && hObject == h.bc.bg.axialSG.d2(1))
    h.b2.bc.r(1,1:2) = 'in'; h.b2.bc.r(2,1:2) = 'os';
    h.b2.bc.rhs.w.r{1} = get(h.bc.et.axialSG.d1.velocity,'string');
    h.b2.bc.rhs.w.r{2} = '@(r)0';
    if ~isempty(h.b2.bc.rhs.w.r{1})
        h.b2.bc.rhs.w.r{1} = convertVelocity(h,h.b2.bc.rhs.w.r{1});
    end

    updateSketch_TFM
    axes(h.bc.as.axialSG.d1(3)); cla(h.bc.as.axialSG.d1(3));
    text(0.94*h.tp_r,0.47,'m/s',h.latex_r{:})
    if get(h.bc.pm.axialSG.d1.velocity,'value') == 2
        text(0.67*h.tp_l,0.47,['$w_\mathrm{g}(' rx(p) ') =$'],h.latex_l{:})
    else
        text(0.67*h.tp_l,0.47,'$\bar{w}_\mathrm{g} =$',h.latex_l{:})
    end
...
elseif (strcmp(selection,'Outflow') && hObject == h.bc.bg.axialSG.d1(1)) || (strcmp(selection,'Inflow') && hObject == h.bc.bg.axialSG.d2(1))
    h.b2.bc.r(1,1:2) = 'os'; h.b2.bc.r(2,1:2) = 'in';
    h.b2.bc.rhs.w.r{1} = '@(r)0';
    h.b2.bc.rhs.w.r{2} = get(h.bc.et.axialSG.d2.velocity,'string');
    if ~isempty(h.b2.bc.rhs.w.r{2})
        h.b2.bc.rhs.w.r{2} = convertVelocity(h,h.b2.bc.rhs.w.r{2});
    end
...
elseif strcmp(selection,'No Penetration')
    ...
    set([h.bc.rb.axialSG.d1.noSlip h.bc.rb.axialSG.d2.noSlip],'value',1,'enable','on')
    set([h.bc.rb.axialSG.d1.slip h.bc.rb.axialSG.d2.slip],'enable','on')
    cla(h.bc.as.axialSG.d1(3),'reset'); set(h.bc.as.axialSG.d1(3),'visible','off')
    cla(h.bc.as.axialSG.d2(3),'reset'); set(h.bc.as.axialSG.d2(3),'visible','off')
...
eval([h.vlNm '=''@(' h.vlNm(end-3) ')' num2str(eval(h.vlNm)) ''';'])
...
switch h.vlNm
    ...
    case 'h.b2.bc.rhs.T.z{1}'
        h.T_d1g = str2double(h.b2.bc.rhs.T.z{1}(5:end));
        
    case 'h.b2.bc.rhs.T.z{3}'
        h.T_d2g = str2double(h.b2.bc.rhs.T.z{3}(5:end));
end

guidata(hObject,h)

function bc_et_velocity(hObject, ~, ~)
h = guidata(hObject);

if eval(h.vlNm) == 0
    switch h.vlNm
        case 'h.b2.bc.rhs.w.r{1}'
            h.w_in = 1;
            set(h.bc.et.axialSG.d1.velocity,'string',num2str(-h.w_in))
        case 'h.b2.bc.rhs.w.r{2}'
            h.w_in = -1;
            set(h.bc.et.axialSG.d2.velocity,'string',num2str(-h.w_in))
    end
    eval([h.vlNm '=''@(r)' num2str(h.w_in) ''';'])
else
    h.w_in = -eval(h.vlNm);
    eval([h.vlNm '=''@(r)' num2str(h.w_in) ''';'])
end
...
switch hObject
    case {h.bc.pm.axialLB.d1.temperature,h.bc.et.axialLB.d1.temperature}
        ...
        switch val
            ...
            case 2
                ...
                h.b1.bc.rhs.T.r{1} = inputTemperature_TFM(rx(p),h.r_c,h.flowopt.ax);
    ...
    case {h.bc.pm.axialLB.d2.temperature,h.bc.et.axialLB.d2.temperature}
        ...
        switch val
            ...
            case 2
                ...
                h.b1.bc.rhs.T.r{2} = inputTemperature_TFM(rx(p),h.r_c,h.flowopt.ax);
    ...
    case {h.bc.pm.radialLB.rc.temperature,h.bc.et.radialLB.rc.temperature}
        ...
        switch val
            ...
            case 2
                ...
                h.b1.bc.rhs.T.z{1} = inputTemperature_TFM(zy(p),h.r_c,h.flowopt.ax);
    ...
    case {h.bc.pm.axialSG.d1.temperature,h.bc.et.axialSG.d1.temperature}
        axes(h.bc.as.axialSG.d1(2)); cla(h.bc.as.axialSG.d1(2));
        text(0.94*h.tp_r,0.47,'$^\circ$C',h.latex_r{:})
        switch val
            case 1
                text(0.4*h.tp_l,0.47,'$T_{1} =$',h.latex_l{:})
                set(h.bc.et.axialSG.d1.temperature,'Enable','on')
                
                if ~isempty(get(h.bc.et.axialSG.d1.temperature,'String'))
                    [~, status]=str2num(get(h.bc.et.axialSG.d1.temperature,'string'));
                    if status == 0
                        set(h.bc.et.axialSG.d1.temperature,'String','')
                        h.b2.bc.rhs.T.r{1} = [];
                    end
                end
            case 2
                text(0.4*h.tp_l,0.47,['$T_{1}(' rx(p) ') =$'],h.latex_l{:})
                set(h.bc.et.axialSG.d1.temperature,'Enable','inactive')
                if contains(get(h.bc.et.axialSG.d1.temperature,'string'),rx(p)) == 0
                    set(h.bc.et.axialSG.d1.temperature,'string','')
                end
                
                h.b2.bc.rhs.T.r{1} = inputTemperature_TFM(rx(p),h.r_c,h.flowopt.ax);
                if isempty(h.b2.bc.rhs.T.r{1}) && contains(get(h.bc.et.axialSG.d1.temperature,'string'),rx(p))
                    h.b2.bc.rhs.T.r{1} = get(h.bc.et.axialSG.d1.temperature,'string');
                end
                set(h.bc.et.axialSG.d1.temperature,'String',h.b2.bc.rhs.T.r{1})
                h.b2.bc.rhs.T.r{1} = convertTemperature(h,h.b2.bc.rhs.T.r{1},'r');
        end
        
    case {h.bc.pm.axialSG.d2.temperature,h.bc.et.axialSG.d2.temperature}
        axes(h.bc.as.axialSG.d2(2)); cla(h.bc.as.axialSG.d2(2));
        text(0.94*h.tp_r,0.47,'$^\circ$C',h.latex_r{:})
        switch val
            case 1
                text(0.4*h.tp_l,0.47,'$T_{2} =$',h.latex_l{:})
                set(h.bc.et.axialSG.d2.temperature,'Enable','on')
                
                if ~isempty(get(h.bc.et.axialSG.d2.temperature,'String'))
                    [~, status]=str2num(get(h.bc.et.axialSG.d2.temperature,'string'));
                    if status == 0
                        set(h.bc.et.axialSG.d2.temperature,'String','')
                        h.b2.bc.rhs.T.r{2} = [];
                    end
                end
            case 2
                text(0.4*h.tp_l,0.47,['$T_{2}(' rx(p) ') =$'],h.latex_l{:})
                set(h.bc.et.axialSG.d2.temperature,'Enable','inactive')
                if contains(get(h.bc.et.axialSG.d2.temperature,'string'),rx(p)) == 0
                    set(h.bc.et.axialSG.d2.temperature,'string','')
                end
                
                h.b2.bc.rhs.T.r{2} = inputTemperature_TFM(rx(p),h.r_c,h.flowopt.ax);
                if isempty(h.b2.bc.rhs.T.r{2}) && contains(get(h.bc.et.axialSG.d2.temperature,'string'),rx(p))
                    h.b2.bc.rhs.T.r{2} = get(h.bc.et.axialSG.d2.temperature,'string');
                end
                set(h.bc.et.axialSG.d2.temperature,'String',h.b2.bc.rhs.T.r{2})
                h.b2.bc.rhs.T.r{2} = convertTemperature(h,h.b2.bc.rhs.T.r{2},'r');
        end
        
    case {h.bc.pm.radialSG.d1.temperature,h.bc.et.radialSG.d1.temperature}
        axes(h.bc.as.radialSG.d1(2)); cla(h.bc.as.radialSG.d1(2));
        text(0.94*h.tp_r,0.47,'$^\circ$C',h.latex_r{:})
        switch val
            case 1
                text(0.4*h.tp_l,0.47,['$T_{' rx(p) '_i} =$'],h.latex_l{:})
                set(h.bc.et.radialSG.d1.temperature,'Enable','on')
                
                if ~isempty(get(h.bc.et.radialSG.d1.temperature,'String'))
                    [~, status]=str2num(get(h.bc.et.radialSG.d1.temperature,'string'));
                    if status == 0
                        set(h.bc.et.radialSG.d1.temperature,'String','')
                        h.b2.bc.rhs.T.z{1} = [];
                    end
                end
            case 2
                text(0.4*h.tp_l,0.47,['$T_{' rx(p) '_i}(' zy(p) ') =$'],h.latex_l{:})
                set(h.bc.et.radialSG.d1.temperature,'Enable','inactive')
                if contains(get(h.bc.et.radialSG.d1.temperature,'string'),zy(p)) == 0
                    set(h.bc.et.radialSG.d1.temperature,'string','')
                end
                
                h.b2.bc.rhs.T.z{1} = inputTemperature_TFM(zy(p),h.r_c,h.flowopt.ax);
                if isempty(h.b2.bc.rhs.T.z{1}) && contains(get(h.bc.et.radialSG.d1.temperature,'string'),zy(p))
                    h.b2.bc.rhs.T.z{1} = get(h.bc.et.radialSG.d1.temperature,'string');
                end
                set(h.bc.et.radialSG.d1.temperature,'String',h.b2.bc.rhs.T.z{1})
                h.b2.bc.rhs.T.z{1} = convertTemperature(h,h.b2.bc.rhs.T.z{1},'z');
        end
        
    case {h.bc.pm.radialSG.d2.temperature,h.bc.et.radialSG.d2.temperature}
        axes(h.bc.as.radialSG.d2(2)); cla(h.bc.as.radialSG.d2(2));
        text(0.94*h.tp_r,0.47,'$^\circ$C',h.latex_r{:})
        switch val
            case 1
                text(0.4*h.tp_l,0.47,['$T_{' rx(p) '_i} =$'],h.latex_l{:})
                set(h.bc.et.radialSG.d2.temperature,'Enable','on')
                
                if ~isempty(get(h.bc.et.radialSG.d2.temperature,'String'))
                    [~, status]=str2num(get(h.bc.et.radialSG.d2.temperature,'string'));
                    if status == 0
                        set(h.bc.et.radialSG.d2.temperature,'String','')
                        h.b2.bc.rhs.T.z{3} = [];
                    end
                end
            case 2
                text(0.4*h.tp_l,0.47,['$T_{' rx(p) '_i}(' zy(p) ') =$'],h.latex_l{:})
                set(h.bc.et.radialSG.d2.temperature,'Enable','inactive')
                if contains(get(h.bc.et.radialSG.d2.temperature,'string'),zy(p)) == 0
                    set(h.bc.et.radialSG.d2.temperature,'string','')
                end
                
                h.b2.bc.rhs.T.z{3} = inputTemperature_TFM(zy(p),h.r_c,h.flowopt.ax);
                if isempty(h.b2.bc.rhs.T.z{3}) && contains(get(h.bc.et.radialSG.d2.temperature,'string'),zy(p))
                    h.b2.bc.rhs.T.z{3} = get(h.bc.et.radialSG.d2.temperature,'string');
                end
                set(h.bc.et.radialSG.d2.temperature,'String',h.b2.bc.rhs.T.z{3})
                h.b2.bc.rhs.T.z{3} = convertTemperature(h,h.b2.bc.rhs.T.z{3},'z');
        end
        
    case {h.bc.pm.radialSG.ro.temperature,h.bc.et.radialSG.ro.temperature}
        axes(h.bc.as.radialSG.ro(2)); cla(h.bc.as.radialSG.ro(2));
        text(0.94*h.tp_r,0.47,'$^\circ$C',h.latex_r{:})
        switch val
            case 1
                text(0.4*h.tp_l,0.47,['$T_{' rx(p) '_o} =$'],h.latex_l{:})
                set(h.bc.et.radialSG.ro.temperature,'Enable','on')
                
                if ~isempty(get(h.bc.et.radialSG.ro.temperature,'String'))
                    [~, status]=str2num(get(h.bc.et.radialSG.ro.temperature,'string'));
                    if status == 0
                        set(h.bc.et.radialSG.ro.temperature,'String','')
                        h.b2.bc.rhs.T.z{4} = [];
                    end
                end
            case 2
                text(0.4*h.tp_l,0.47,['$T_{' rx(p) '_o}(' zy(p) ') =$'],h.latex_l{:})
                set(h.bc.et.radialSG.ro.temperature,'Enable','inactive')
                if contains(get(h.bc.et.radialSG.ro.temperature,'string'),zy(p)) == 0
                    set(h.bc.et.radialSG.ro.temperature,'string','')
                end
                
                h.b2.bc.rhs.T.z{4} = inputTemperature_TFM(zy(p),h.r_c,h.flowopt.ax);
                if isempty(h.b2.bc.rhs.T.z{4}) && contains(get(h.bc.et.radialSG.ro.temperature,'string'),zy(p))
                    h.b2.bc.rhs.T.z{4} = get(h.bc.et.radialSG.ro.temperature,'string');
                end
                set(h.bc.et.radialSG.ro.temperature,'String',h.b2.bc.rhs.T.z{4})
                h.b2.bc.rhs.T.z{4} = convertTemperature(h,h.b2.bc.rhs.T.z{4},'z');
        end
end
...
function bc_pm_velocity(hObject, ~, ~)
h = guidata(hObject);
...
switch hObject
    case {h.bc.pm.axialSG.d1.velocity,h.bc.et.axialSG.d1.velocity}
        axes(h.bc.as.axialSG.d1(3)); cla(h.bc.as.axialSG.d1(3));
        text(0.94*h.tp_r,0.47,'m/s',h.latex_r{:})
        switch val
            case 1
                text(0.67*h.tp_l,0.47,'$\bar{w}_\mathrm{g} =$',h.latex_l{:})
                set(h.bc.et.axialSG.d1.velocity,'Enable','on')
                
                if ~isempty(get(h.bc.et.axialSG.d1.velocity,'String'))
                    [~, status]=str2num(get(h.bc.et.axialSG.d1.velocity,'string'));
                    if status == 0
                        set(h.bc.et.axialSG.d1.velocity,'String','')
                        h.b2.bc.rhs.w.r{1} = [];
                    end
                end
            case 2
                ...
                if contains(get(h.bc.et.axialSG.d1.velocity,'string'),rx(p)) == 0
                    set(h.bc.et.axialSG.d1.velocity,'string','')
                end
                
                h.b2.bc.rhs.w.r{1} = inputVelocity(rx(p));
                if isempty(h.b2.bc.rhs.w.r{1}) && contains(get(h.bc.et.axialSG.d1.velocity,'string'),rx(p))
                    h.b2.bc.rhs.w.r{1} = get(h.bc.et.axialSG.d1.velocity,'string');
                end
                set(h.bc.et.axialSG.d1.velocity,'String',h.b2.bc.rhs.w.r{1})
                h.b2.bc.rhs.w.r{1} = convertVelocity(h,h.b2.bc.rhs.w.r{1});
        end
        
    case {h.bc.pm.axialSG.d2.velocity,h.bc.et.axialSG.d2.velocity}
        axes(h.bc.as.axialSG.d2(3)); cla(h.bc.as.axialSG.d2(3));
        text(0.94*h.tp_r,0.47,'m/s',h.latex_r{:})
        switch val
            case 1
                text(0.67*h.tp_l,0.47,'$\bar{w}_\mathrm{g} =$',h.latex_l{:})
                set(h.bc.et.axialSG.d2.velocity,'Enable','on')
                
                if ~isempty(get(h.bc.et.axialSG.d2.velocity,'String'))
                    [~, status]=str2num(get(h.bc.et.axialSG.d2.velocity,'string'));
                    if status == 0
                        set(h.bc.et.axialSG.d2.velocity,'String','')
                        h.b2.bc.rhs.w.r{2} = [];
                    end
                end
            case 2
                ...
                if contains(get(h.bc.et.axialSG.d2.velocity,'string'),rx(p)) == 0
                    set(h.bc.et.axialSG.d2.velocity,'string','')
                end
                
                h.b2.bc.rhs.w.r{2} = inputVelocity(rx(p));
                if isempty(h.b2.bc.rhs.w.r{2}) && contains(get(h.bc.et.axialSG.d2.velocity,'string'),rx(p))
                    h.b2.bc.rhs.w.r{2} = get(h.bc.et.axialSG.d2.velocity,'string');
                end
                set(h.bc.et.axialSG.d2.velocity,'String',h.b2.bc.rhs.w.r{2})
                h.b2.bc.rhs.w.r{2} = convertVelocity(h,h.b2.bc.rhs.w.r{2});
        end
...
switch selection
    case 'Fluids at rest. Constant mean temperature.'
...
% changing button options
    ...
    set([h.tabs{:} h.sy.et.n_eig h.sy.et.n_eig_c h.sy.et.m_start h.sy.et.m_end h.sy.rb.delta_T h.sy.rb.T_d1 h.sy.rb.T_d2 h.sy.pb.run(1) h.sy.pb.run(2) h.sy.pb.load h.sy.pb.save h.sy.pb.plot],'Enable','inactive')
...
% show axes for eigenvalues
    ...
    updateValues_TFM; evalin('base','caseSetup_TFM_V3d2') % problem_specification
    
% run simulation
    evalin('base','gamma_m = []; m_s =  m_start:m_end;')
    i = 0;
    for m = h.m_start:h.m_delta:h.m_end
        i = i+1;
    ...
    if hObject == h.sy.pb.run(1)
        ...
        set([h.tabs{:} h.sy.et.n_eig h.sy.et.n_eig_c h.sy.et.m_start h.sy.et.m_end h.sy.rb.delta_T h.sy.rb.T_d1 h.sy.rb.T_d2 h.sy.pb.run(1) h.sy.pb.run(2) h.sy.pb.load h.sy.pb.save h.sy.pb.plot],'Enable','on')
...
% equal during search for critical mode
    ...
    T_d1g = str2double(h.b2.bc.rhs.T.z{1}(5:end));
    T_d2g = str2double(h.b2.bc.rhs.T.z{3}(5:end));
    ...
    T_list = {'T_d1l','T_d2l','T_d1g','T_d2g','T_p1','T_p2','T_ro','T_rc'};
    T_s = [T_d1l, T_d2l, T_d1g, T_d2g, T_p1, T_p2, T_ro, T_rc];
...
% if gamma_m exists, one can skip the first eigenvalue computation
    if evalin('base','exist(''gamma_m'',''var'')') == 0
...
% compute gamma
    if min(real(h.gamma_m))<0
        ...
        if get(h.sy.pb.stop,'userdata')
            ...
            et_T_equal = [h.bc.et.axialLB.d1.temperature h.bc.et.axialLB.d2.temperature h.bc.et.radialSG.d1.temperature h.bc.et.radialSG.d2.temperature h.bc.et.axialSG.d1.temperature h.bc.et.axialSG.d2.temperature h.bc.et.radialSG.ro.temperature h.bc.et.radialLB.rc.temperature];
            for i = 1:length(T_s)
                if T_d1l == T_s(i) && ~strcmp(h.dependent,'T_d2l')
