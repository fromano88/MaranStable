...
delta_x = 0.3;
delta_y = 0.65;
rectangle('Position',[-delta_x -delta_y r_i+delta_r+2*delta_x l_ges+2*delta_y],'FaceColor',[1 1 1],'LineStyle','none')
hold on
...
i = 1;
while i*r_i/n.lines < l_ges
    plot(r_i+delta_r+[0 r_i/n.lines],r_i/n.lines*[i-1 i],'lineWidth',w*0.25,'color','k')
    i = i+1;
end
...
% gravitiy vector
if g ~= 0
    plot(r_i+0.8*delta_r*[1 1],l_d2+l_lb+0.2*l_d1-[0 l.g],'lineWidth',1.2*w,'color','k')
    if g > 0
        fill(r_i+0.8*delta_r+d*[-1 0 1],l_d2+l_lb+0.2*l_d1-l.g-[0 3*d 0],'k','lineWidth',1.2*w)
    else
        fill(r_i+0.8*delta_r+d*[-1 0 1],l_d2+l_lb+0.2*l_d1+[0 3*d 0],'k','lineWidth',1.2*w)
    end
end
...
if strcmp(h.b2.bc.r(1,1),'w')
    ...
    while i*r_i/n.lines < delta_r
        plot(r_i+r_i/n.lines*[i-1 i],[-r_i/n.lines 0],'lineWidth',w*0.25,'color','k')
        i = i+1;
    end
    i = 1;
    while i*r_i/n.lines < delta_r
        plot(r_i+r_i/n.lines*[i-1 i],l_ges+[0 r_i/n.lines],'lineWidth',w*0.25,'color','k')
        i = i+1;
    end
else
    ...
    plot(r_i+delta_r/2*[1 1],l_ges+l.in/2*[-1 1],'lineWidth',1.5*w,'color','k')
    if strcmp(h.b2.bc.r(1,1),'i')
        fill(r_i+0.5*delta_r+d*[-1 0 1],-l.in/2-[0 3*d 0],'k','lineWidth',1.5*w)
        fill(r_i+0.5*delta_r+d*[-1 0 1],l_ges-l.in/2-[0 3*d 0],'k','lineWidth',1.5*w)
    else
        fill(r_i+0.5*delta_r+d*[-1 0 1],l.in/2+[0 3*d 0],'k','lineWidth',1.5*w)
        fill(r_i+0.5*delta_r+d*[-1 0 1],l_ges+l.in/2+[0 3*d 0],'k','lineWidth',1.5*w)
    end
